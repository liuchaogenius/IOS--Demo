...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError * __autoreleasing *error) {
    ...
    aspect_performLocked(^{
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) //判断能否被hook
        {
            ...
            if (identifier) { //放到container中，他是被hook类的一个属性
...
static BOOL aspect_isCompatibleBlockSignature(NSMethodSignature *blockSignature, id object, SEL selector, NSError **error) {
    ...
    if (blockSignature.numberOfArguments > methodSignature.numberOfArguments) {
        signaturesMatch = NO;
    }else { //判断block 签名一定要有id<aspectinfo>参数
        if (blockSignature.numberOfArguments > 1) {
            const char *blockType = [blockSignature getArgumentTypeAtIndex:1];
            if (blockType[0] != '@') { //  判断block 参数类型
                signaturesMatch = NO;
            }
        }
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    NSCParameterAssert(self); //如果self是实例对象  self.class与object_getClass 一样，如果是类对象一个自己一个元类
...
// Loads or creates the aspect container. 创建一个aspects__viewWillAppear: 将AspectsContainer 添加到当前类中
...
+ (instancetype)identifierWithSelector:(SEL)selector object:(id)object options:(AspectOptions)options block:(id)block error:(NSError **)error {
    NSCParameterAssert(block);
    NSCParameterAssert(selector); //先做一个block函数签名
    NSMethodSignature *blockSignature = aspect_blockMethodSignature(block, error); // TODO: check signature compatibility, etc.
    if (!aspect_isCompatibleBlockSignature(blockSignature, object, selector, error)) {
        return nil; //判断当前block签名与selector签名是否合法
    }
