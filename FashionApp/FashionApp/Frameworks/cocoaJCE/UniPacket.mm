...
- (NSString *)description
{
    ...
    result = [NSString stringWithFormat:@"\n--------------- UniPacket ---------------\n"
              ...
              self.context,
              self.status,
              self.attributes.jce_uniAttributes];
    return result;
}

//- (NSString *)objectKeyWithName:(NSString *)name
//{
//    NSDictionary *object = CZ_ValueForKey(self.attributes.jce_uniAttributes, name);
...
- (void)__unpack:(JceInputStreamV2 *)stream
{
    @autoreleasepool {
        ...
        _attributes = [UniAttribute fromAttributeData:self.sBuffer];
...
{
    ASSERT_TRHOW_WS_EXCEPTION([theClass isSubclassOfClass:[JceObject class]]);
