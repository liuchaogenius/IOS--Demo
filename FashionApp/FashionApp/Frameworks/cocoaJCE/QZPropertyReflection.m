...
#define __FILE__ "QZPropertyReflection"
#pragma clang diagnostic pop

//
//  QZPropertyReflection.m
//
...
/**
 *  返回一个保存 Objective-C property 元信息的 QZPropertyAttributes 结构体
 *  @note 返回的结构体在使用完后需要手动调用 free()
 */
QZPropertyAttributes * QZCopyPropertyAttributes(objc_property_t property) {
    if (!property) {
        return NULL;
    }
    ...
    QZPropertyAttributes *attributes = calloc(1, sizeof(QZPropertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate QZPropertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = QZPropertyMemoryManagementPolicyCopy;
                break;
                
            case '&':
                attributes->memoryManagementPolicy = QZPropertyMemoryManagementPolicyRetain;
                break;
...
SEL QZSelectorWithCapitalizedKeyPattern(const char *prefix, NSString *key, const char *suffix) {
